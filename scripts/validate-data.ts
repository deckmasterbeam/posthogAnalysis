...
check(
  "has issuesByCloser object",
  typeof d.issuesByCloser === "object" && !Array.isArray(d.issuesByCloser)
);
...
check(
  "analysis window is ~90 days",
  windowDays >= 85 && windowDays <= 95,
  `${windowDays.toFixed(1)} days`
);
...
if (Array.isArray(d.prDetails)) {
  ...
  check(
    "all PRs have required fields",
    missingFields.size === 0,
    missingFields.size > 0 ? `missing: ${[...missingFields].join(", ")}` : undefined
  );
  check(
    "all PRs have mergedAt > createdAt",
    badDates === 0,
    badDates > 0 ? `${badDates} violations` : undefined
  );
  check(
    "all PRs are within the analysis window",
    outOfWindow === 0,
    outOfWindow > 0 ? `${outOfWindow} out of window` : undefined
  );
  check(
    "no PR author appears as their own reviewer",
    selfReviews === 0,
    selfReviews > 0 ? `${selfReviews} self-reviews` : undefined
  );
...
if (typeof d.issuesByCloser === "object" && d.issuesByCloser !== null) {
  const badEntries = Object.entries(d.issuesByCloser as Record<string, unknown>).filter(
    ([, v]) => typeof v !== "number" || (v as number) <= 0
  );
  check(
    "issuesByCloser values are positive numbers",
    badEntries.length === 0,
    badEntries.length > 0 ? `${badEntries.length} bad entries` : undefined
  );
}
